...
BEGIN
  ...
  -- Score tendance (last 3 vs first 3)
  SELECT array_agg(score ORDER BY created_at DESC) INTO _recent_scores
  FROM (SELECT score, created_at FROM resultats WHERE eleve_id = _eleve_id ORDER BY created_at DESC LIMIT 3) t;
  SELECT array_agg(score ORDER BY created_at ASC) INTO _old_scores
  FROM (SELECT score, created_at FROM resultats WHERE eleve_id = _eleve_id ORDER BY created_at ASC LIMIT 3) t;
  ...
  -- Score critique
  SELECT array_agg(score ORDER BY created_at DESC) INTO _recent_scores
  FROM (SELECT score, created_at FROM resultats WHERE eleve_id = _eleve_id ORDER BY created_at DESC LIMIT 5) t;
...
CREATE OR REPLACE TRIGGER trg_recalculate_score_risque
  AFTER INSERT ON public.resultats
  FOR EACH ROW
  EXECUTE FUNCTION public.recalculate_score_risque();
