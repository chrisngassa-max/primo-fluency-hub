...
import { getSliceTargetLevel } from "../_shared/differentiation/types.ts";
import {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: family, error } = await admin.from("differentiation_families")
      .select("id, source_id, source_content_hash, created_by, generation_status, validation_status, review_status, published_exercise_id, payload, target_level, referential_version")
      .eq("id", body.familyId)
      .maybeSingle();
    ...
    if (family.published_exercise_id) {
      return json(409, { error: "FAMILY_ALREADY_PUBLISHED", exercise_id: family.published_exercise_id });
    }
    if (
      family.generation_status !== "generated"
      || !["passed", "passed_with_warnings"].includes(family.validation_status)
      || family.review_status !== "validated"
    ) {
    ...
    const { data: source } = await admin
      ...
      .maybeSingle();
    if (!source || source.content_hash !== family.source_content_hash) {
      return json(422, { error: "SOURCE_HASH_DIVERGED" });
    }
    ...
    const defaultPoint = pickDeterministicCoMasteryPoint(masteryPoints ?? [], targetLevel);
    ...
    const audioRef = source.source_kind === "audio"
      ? {
        ...
        source_content_hash: family.source_content_hash,
    ...
    const { data: exercise, error: insertError } = await admin.from("exercices")
      .insert(familyVariantToExerciceRow(
        ...
        defaultPoint.id,
        audioRef,
        family.referential_version ?? null,
      ))
      .select("id")
      .single();
    ...
    const { data: publishedFamily, error: updateError } = await admin.from("differentiation_families").update({
      ...
      published_exercise_id: exercise.id,
    ...
    if (!publishedFamily) {
      ...
      const { data: alreadyPublished } = await admin.from("differentiation_families")
        .select("published_exercise_id")
        .eq("id", family.id)
        .maybeSingle();
      return json(409, {
        error: "FAMILY_ALREADY_PUBLISHED",
        exercise_id: alreadyPublished?.published_exercise_id,
      });
    }
