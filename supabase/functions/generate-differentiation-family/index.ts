...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const facts: DifferentiationFact[] = (Array.isArray(factResponse.facts) ? factResponse.facts : []).map((fact: any, index: number) => ({
      fact_id: typeof fact.fact_id === "string" ? fact.fact_id : `F${index + 1}`,
      ...
      provenance: { source_id: source.id, transcription_id: transcription.id, segment_refs: Array.isArray(fact.segment_refs) ? fact.segment_refs : [], chunk_refs: Array.isArray(fact.chunk_refs) ? fact.chunk_refs : [], quote: String(fact.quote ?? "") },
